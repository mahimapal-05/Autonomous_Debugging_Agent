...
with st.sidebar:
    ...
    if gemini_active:
        ...
        st.caption("Operating with intelligent multi-pass AST & AST rule-based self-correction engine.")
...
st.markdown('<div class="sub-header">Multi-Agent AI Pipeline + Web-Based Python Interpreter & Self-Correction Engine</div>', unsafe_allow_html=True)
...
with tab_interpreter:
    ...
    with int_col1:
        ...
        with act_col1:
            run_interp_btn = st.button("▶ Run in Interpreter", type="primary", use_container_width=True)
        with act_col2:
            debug_from_interp_btn = st.button("🩺 Auto-Debug with AI Agent", type="secondary", use_container_width=True)
    ...
    with int_col2:
        ...
        if run_interp_btn:
            ...
            if exec_res["success"]:
                ...
                stat_c1.error(f"❌ **{exec_res.get('error_type', 'Error')} Detected**")
            stat_c2.metric("Execution Time", f"{exec_res['execution_time_ms']} ms")
            ...
            if exec_res["stderr"]:
                ...
                if exec_res.get("lineno"):
                    st.warning(f"📍 Exception occurred at **Line {exec_res['lineno']}**")

            # Variables State Inspector
            if exec_res.get("variables"):
                with st.expander("🔍 Variable State Inspector", expanded=True):
                    for var_name, var_info in exec_res["variables"].items():
                        st.markdown(f"- `{var_name}` (*{var_info['type']}*): `{var_info['value']}`")

        elif debug_from_interp_btn:
            # Auto-run first to capture error
            exec_res = execute_python_code(interp_code_val)
